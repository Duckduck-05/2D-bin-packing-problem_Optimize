...
chrono::time_point<chrono::system_clock> st, en;
bool flag = false;
...
{
    // input
    vector<pair<int, int>> numTest = {{1, 40}, {0, 59}, {0, 59}};
    for (int phase = 1; phase <= 3; ++phase) {
        for (int test = numTest[phase - 1].first; test <= numTest[phase - 1].second; ++test) {
            string input = "Test_case/Phase_" + to_string(phase) + "/input" + string(test <= 9 ? "0" : "") + to_string(test) + ".txt";
            string output = "Output/Output-BFS/Phase_" + to_string(phase) + "/output" + string(test <= 9 ? "0" : "") + to_string(test) + ".txt";
            // cout << input << ' ' << output << '\n';
            freopen(input.c_str(), "r", stdin);
            ofstream cout(output.c_str());
            ...
            for_x(pos, 1, k) if (flag == false) best_first_search(pos); // tim candidate cho pos
            ...
            if (flag == true) {
                ...
                cout << n << ' ' << k << ' ' << total_cost << ' ' << duration.count();
            }
